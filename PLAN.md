...
-   **작업 방식**:
    ...
    -   **`ORDER.md` 갱신 시, 파일 최상단에 해당 시점까지의 모든 변경 사항을 요약한 `git add . && git commit -m "..."` 명령어를 한국어로 제공합니다. 이 메시지는 바로 복사하여 사용할 수 있도록 합니다.**
    -   **`ORDER.md`에 코드를 요청할 때, `4. 외부 AI를 위한 프로젝트 컨텍스트 및 디자인 지침` 섹션의 내용을 항상 포함하여 외부 AI가 프로젝트의 컨텍스트를 명확히 이해하도록 합니다.**
